...
def fedavgm_strategy(model, client_config, dataset_name, strategy_name, meta_action_type, seed=42, server_learning_rate=1.0, server_momentum=0.0):
    """
    Implement FedAvgM Strategy
    """
    initial_parameters = fl.common.ndarrays_to_parameters(get_parameters(model)) #set intial parameters

    class SaveFedAvgMModelStrategy(fl.server.strategy.FedAvgM):
        def aggregate_fit(
            self,
            server_round: int,
    ...
    strategy = SaveFedAvgMModelStrategy(
        fraction_fit = 0.8,
